...
describe('VariableDeclaration', () => {
  test('init Literal', () => {
    let code = `var a = 1`;
    expect(inject(code, { minify: false }).trim()).toBe('var a = e_user_hook("a", 1, "var-init");');
    code = `const a = "abc"`;
    expect(inject(code, { minify: false }).trim()).toBe(`const a = e_user_hook("a", "abc", "var-init");`);
    code = `const a = obj.b.c;`;
    expect(inject(code, { minify: false }).trim()).toBe(
  ...
  console.log(a, b);
    ...
    `;
    expect(inject(code, { minify: true })).toBe(
...
describe('FunctionDeclaration', () => {
  ...
  console.log(a);
    ...
    // console.log(inject(code, { minify: true }));
  });
  test('RestElement', () => {
